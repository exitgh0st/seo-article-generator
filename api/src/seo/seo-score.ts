 ...
 *
 * A TypeScript port of scripts/seo-score.mjs, kept behaviourally identical — the
 * committed JSON under app/public/content/articles/ is the regression fixture
 * (see seo-score.spec.ts). The .mjs original stays in place so the Claude Code
 * CLI workflow keeps working without a running server; this is the copy the API
 * and the generation pipeline score against.
 *
 * Returns { score, max, rows: [{ n, name, status, detail, required }] }.
 * status is 'pass' | 'fail' | 'warn'. Only 'pass' earns a point.
 */

import type { ArticleFrontmatter, Heading, SeoResult, SeoRow } from './article.types';
...
const QUESTION_STARTERS =
  /^(what|why|how|who|when|where|which|is|are|does|do|can|should|will)\b/i;
...
export function extractHeadings(markdown: string): Omit<Heading, 'id'>[] {
...
function fleschReadingEase(prose: string): number | null {
  const sentences = splitSentences(prose);
  const words: string[] = prose.match(/\b[\w'-]+\b/g) ?? [];
...
function extractLinks(markdown: string): { internal: Link[]; external: Link[] } {
  const internal: Link[] = [];
  const external: Link[] = [];
  const re = /(?<!!)\[([^\]]*)\]\(([^)\s]+)/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(markdown)) !== null) {
    ...
    else if (href.startsWith('/article/') || href.endsWith('.md'))
...
export interface ScoreContext {
  /** Used to relax the internal-link row while the site has nothing to link to. */
...
export function scoreArticle(
  ...
  row(
    ...
    `${title.length} chars, keyword ${kwInTitleHead ? 'in' : 'NOT in'} first 5 words`,
    true,
  );
  ...
  row(
    2,
    'H1',
    h1s.length === 0 ? 'pass' : 'fail',
    h1s.length === 0
      ? 'one, from frontmatter'
      : `${h1s.length} in body — move to the \`headline\` field`,
    true,
  );
  ...
  const descOk =
    desc.length >= 150 && desc.length <= 160 && countPhrase(desc, keyword) > 0;
  row(
    3,
    'Meta description',
    descOk ? 'pass' : 'fail',
    `${desc.length} chars (150–160), keyword ${countPhrase(desc, keyword) > 0 ? '✓' : '✗'}`,
    true,
  );
  ...
  row(
    ...
    'Slug',
    slugOk ? 'pass' : 'fail',
    `${slugParts.length} words${slugStops.length ? `, stopwords: ${slugStops.join(', ')}` : ''}${slugHasDate ? ', contains a date' : ''}`,
    true,
  );
  ...
  const kwInH2 = headings.some(
    (h) => h.level === 2 && countPhrase(h.text, keyword) > 0,
  );
  row(
    5,
    'Keyword placement',
    kwInIntro && kwInH2 ? 'pass' : 'fail',
    `first 100 words ${kwInIntro ? '✓' : '✗'}, H2 ${kwInH2 ? '✓' : '✗'}`,
    true,
  );
  ...
  row(
    7,
    'Secondary keywords',
    secondary.length >= 3 && secondary.length <= 5 && !unused.length
      ? 'pass'
      : 'warn',
  ...
  const hasQuestionH2 = headings.some(
    (h) =>
      h.level === 2 && (h.text.includes('?') || QUESTION_STARTERS.test(h.text)),
  );
  row(
    8,
    'Heading hierarchy',
    !skipped && hasQuestionH2 ? 'pass' : 'warn',
    skipped
      ? `skipped level: ${skipped}`
      : hasQuestionH2
        ? 'clean, question-form H2 ✓'
        : 'clean, but no question-form H2',
  );
  ...
  row(
    9,
    'Length',
    wordCount >= 900 && wordCount <= 1400 ? 'pass' : 'warn',
  ...
  row(
    10,
    'Internal links',
    internal.length >= 2 && internal.length <= 4
      ? 'pass'
      : soleArticle
        ? 'pass'
        : 'warn',
    soleArticle && internal.length === 0
      ? '0 — no other articles exist yet'
      : `${internal.length} (target 2–4)`,
  );
  ...
  row(
    11,
    'External links',
    external.length >= 3 ? 'pass' : 'fail',
    `${external.length} (minimum 3)`,
    true,
  );
  ...
  row(
    ...
    hero?.src
      ...
      : 'no hero image declared',
  );

  // 13. Schema — everything NewsArticle JSON-LD needs
  const schemaMissing: string[] = (
    ['title', 'description', 'publishedAt', 'updatedAt'] as const
  ).filter((f) => !data[f]);
  if (!data.author?.name) schemaMissing.push('author.name');
  row(
    13,
    'Schema',
    schemaMissing.length ? 'fail' : 'pass',
    schemaMissing.length ? `missing: ${schemaMissing.join(', ')}` : 'complete',
    true,
  );

  // 14. Featured snippet — a 40-60 word paragraph directly under a question-form H2
  let snippet: { heading: string; words: number } | null = null;
  ...
  row(
    ...
    snippet ? 'pass' : 'warn',
    snippet
      ? `${snippet.words} words under "${snippet.heading}"`
      : 'no 40–60 word answer under a question-form H2',
  );
  ...
  row(
    15,
    'Readability',
    avgSentence < 22 && flesch !== null && flesch >= 40 && flesch <= 65
      ? 'pass'
      : 'warn',
